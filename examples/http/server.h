...
protected:
  ...
  nostd::string server_url_;
  uint16_t port_;
  std::atomic<bool> is_running_{false};

public:
  HttpServer(nostd::string server_name = "test_server", uint16_t port = 8800) : port_(port)
  {
    server_.setServerName(server_name);
    server_.setKeepalive(false);
  }

  void AddHandler(nostd::string path, HTTP_SERVER_NS::HttpRequestCallback *request_handler)
  {
